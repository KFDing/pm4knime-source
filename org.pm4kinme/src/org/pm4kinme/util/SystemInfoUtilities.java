...
/**
 * Helper class to get system information. Copied to pm4knime due to the "tight"
 * connection with RapidProM
 *
 * @author Nils Woehler
 */
public class SystemInfoUtilities {
	...
	/**
	 * Returns total physical memory in MB. If it is run in a 32-bit JVM it may
	 * return only a maximum of 4GB even if more memory is available.
	 *
	...
	/**
	 * Returns free physical memory in MB. If it is run in a 32-bit JVM it may
	 * return only a maximum of 2GB even if more memory is available.
	 * <p>
	 * <strong>Attention:</strong> The returned value is dependant on the OS. For
	 * example on Linux you might see less memory than there is actually available
	 * due to the OS using it for disk caching. That memory is not actually blocked
	 * because the OS would release it if requested, however this method will return
	 * the free memory which does NOT contain the ram used for disk caching. Use
	 * with care!
	 * </p>
	 *
	 * @return the free phsycial memory in MB or <code>-1</code> if we cannot
	 *         determine it
	 * @since 6.0.004
	 *
	 */
	public static Long getFreePhysicalMemorySize() {
		...
		try {
			...
			// in this case we behave as before 6.0.004 where we did not take free memory
			// into
			// account
			return -1L;
		}
	...
	/**
	 * Uses the logger to write current environment infos to the log (with
	 * {@link Level#INFO}).
	 */
	public static void logEnvironmentInfos() {
		if (LOGGER.isLoggable(Level.INFO)) {
			LOGGER.log(Level.INFO,
					"Operating system: " + getOperatingSystemName() + ", Version: " + getOperatingSystemVersion());
