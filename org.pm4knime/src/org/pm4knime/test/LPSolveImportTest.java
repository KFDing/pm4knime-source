...
/**
 * this class tests the lpsolve in the use of PNetReplayer of alignment use. It creates a Petri net and
 * an event log. Then we define the parameters to use it
 * @author kefang-pads
 * @see https://svn.win.tue.nl/repos/prom/Packages/PNetReplayer/Trunk/tests/src-test/org/processmining/tests/newpackage/PNReplayerTestWithoutGUI.java
 */
public class LPSolveImportTest {
	
	@Test
	public static void replayerTest() throws Throwable {
		...
		System.out.println("Result Size : " + pnRepResult.size());
		
		validateResult(pnRepResult);
		System.out.println("Trace Size : " + pnRepResult.iterator().next().getTraceIndex().size());
		...
		System.out.println("Result Size : " + pnRepResult2.size());
		validateResult(pnRepResult2);
		*/
		
		// select the ILP Solver replay
		PetrinetReplayerWithILP replWithILP = new PetrinetReplayerWithILP();
		PNRepResult pnRepResult = replayer.replayLog(null, net, log, mapping, replWithILP, parameter);
		System.out.println("Result Size : " + pnRepResult.size());
		
		// conclusion, here we can load the ILP replayer to solve the question.. I just choose dll as native library
...
//						System.out.print("(M)");
	...
	// show the result of above codes
	public static void main(String[] args) {
		try {
			// LPSolveImportTest.replayerTest();
			
			String path = System.getProperty("java.library.path");
			// System.out.println(path.indexOf("dll"));
			System.out.println(path);
			System.loadLibrary("lpsolve55");
			System.loadLibrary("lpsolve55j");
			System.out.println("LpSolve Libraries Loaded");
