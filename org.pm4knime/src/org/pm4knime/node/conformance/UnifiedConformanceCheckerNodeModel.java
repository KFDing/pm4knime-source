...
/**
 * <code>NodeModel</code> for the "UnifiedConformanceChecker" node.
 * It is designed for workflow in loop without the option from the event log and Petri net. 
 * 
 * @author Kefang Ding
 */
public class UnifiedConformanceCheckerNodeModel extends NodeModel {
	...
	private SettingsModelIntegerBounded[] m_defaultCostModels; 
    ...
    public static SettingsModelIntegerBounded[] initializeCostModels() {
    	...
    	SettingsModelIntegerBounded[] defaultCostModels = new SettingsModelIntegerBounded[CFG_COST_TYPE_NUM];
    	int i=0;
    ...
    protected PortObject[] execute(final PortObject[] inData,
    	...
    	int[] defaultCosts = {m_defaultCostModels[0].getIntValue(), m_defaultCostModels[1].getIntValue(), m_defaultCostModels[2].getIntValue()};
		XEventClassifier eventClassifier = getXEventClassifier();
		IPNReplayParameter parameters =  getParameters(log, anet, eventClassifier, defaultCosts);
		...
		// the order has changed, we should define the unified model at first, and then the customized model
		// but now, next time think simpler at first
		BufferedDataTable bt = ConformanceCheckerNodeModel.createInfoTable(result.getInfo(), exec);
    ...
    private IPNReplayParameter getParameters(XLog log, AcceptingPetriNet anet, XEventClassifier eventClassifier, int[] defaultCosts) {
